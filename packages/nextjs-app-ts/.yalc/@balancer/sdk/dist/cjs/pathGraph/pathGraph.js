...
class PathGraph {
    ...
    sortAndFilterPaths(paths) {
        const pathsWithLimits = paths
            .map(path => {
            const limit = this.getLimitAmountSwapForPath(path, types_1.SwapKind.GivenIn);
            return { path, limit };
        })
    ...
    filterVolatilePools(poolAddresses) {
        ...
        for (const poolAddress of poolAddresses) {
            if (this.poolAddressMap.get(poolAddress)?.poolType === types_1.PoolType.Weighted) {
    ...
    getLimitAmountSwapForPath(path, swapKind) {
        ...
        for (let i = path.length - 2; i >= 0; i--) {
            const poolLimitExactIn = path[i].pool.getLimitAmountSwap(path[i].tokenIn, path[i].tokenOut, types_1.SwapKind.GivenIn);
            const poolLimitExactOut = path[i].pool.getLimitAmountSwap(path[i].tokenIn, path[i].tokenOut, types_1.SwapKind.GivenOut);
            if (poolLimitExactOut <= limit) {
                limit = poolLimitExactIn;
            }
            else {
                const pulledLimit = path[i].pool.swapGivenOut(path[i].tokenIn, path[i].tokenOut, entities_1.TokenAmount.fromRawAmount(path[i].tokenOut, limit)).amount;
